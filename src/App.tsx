...
import { LiveStatusPanel } from './components/app/LiveStatusPanel'
...
function App() {
  ...
  const [brokerUrl, setBrokerUrl] = useState(import.meta.env.VITE_MQTT_URL ?? '')
  ...
  const [gatewaySn, setGatewaySn] = useState(import.meta.env.VITE_GATEWAY_SN ?? '')
  const [deviceSn, setDeviceSn] = useState(import.meta.env.VITE_DEVICE_SN ?? '')
  ...
  const activeEntry =
    psdkEntries.find((entry) => entry.psdk_index === psdkIndex) ?? psdkEntries[0]
